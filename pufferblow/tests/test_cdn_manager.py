...
class TestCDNManagerCategorization:
    ...
    def test_categorize_file_word_doc(self, cdn_manager):
        ...
        category = cdn_manager.categorize_file("doc.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
        assert category == "documents"
    ...
    def test_categorize_file_unknown(self, cdn_manager):
        ...
        category = cdn_manager.categorize_file("unknown.xyz", "application/octet-stream")
    ...
    def test_validate_and_save_categorized_file_invalid_extension(self, cdn_manager):
        ...
        file = UploadFile(filename="test.txt", file=image_buffer)  # TXT is not in IMAGE_EXTENSIONS
    ...
    def test_enhanced_cleanup_all_categories(self, cdn_manager):
        ...
        categories = ['files', 'images', 'avatars', 'banners', 'gifs', 'stickers',
        ...
        for category in categories:
            ...
            test_path = Path(cdn_manager.config.CDN_STORAGE_PATH) / category / f"test_{category}.txt"
            test_path.parent.mkdir(parents=True, exist_ok=True)
            test_path.write_bytes(test_content)

            # Create URL for the database file list simulation
            test_files[category] = [f"{cdn_manager.config.CDN_BASE_URL}/{category}/{test_path.name}"]
...
class TestCDNManagerFileServing:
    ...
    def test_cdn_directory_structure(self, cdn_manager):
        ...
        categories_to_test = ['avatars', 'banners', 'images', 'gifs', 'stickers', 
                             'videos', 'audio', 'documents', 'config', 'files']
