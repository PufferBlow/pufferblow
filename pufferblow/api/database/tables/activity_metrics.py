...
class ActivityMetrics(Base):
    ...
    messages_this_period: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False
    )
    messages_per_hour: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    channels_used: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False
    )  # Number of channels with messages

    # Engagement metrics
    user_engagement_rate: Mapped[float] = mapped_column(
        Float, default=0.0, nullable=False
    )  # % of users who are active
    messages_per_active_user: Mapped[float] = mapped_column(
        Float, default=0.0, nullable=False
    )
    channel_utilization_rate: Mapped[float] = mapped_column(
        Float, default=0.0, nullable=False
    )  # % of channels used
    ...
    server_uptime_hours: Mapped[float] = mapped_column(
        Float, default=0.0, nullable=False
    )
