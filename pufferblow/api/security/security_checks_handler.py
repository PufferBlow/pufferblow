...
class SecurityChecksHandler(object):
    """
    This class handles all the security checks
    """
    def __init__(self, database_handler: DatabaseHandler, channels_manager: ChannelsManager, user_manager: UserManager, auth_token_manager: AuthTokenManager) -> None:
    ...
    def check_user_id(self, user_id: str) -> None:
        ...
        Returns:
            None.
        """
        if not self.user_manager.check_user(
            user_id=user_id
        ):
    ...
    def check_user(self, auth_token: str | None = None, user_id: str | None = None) -> None:
        """
    ...
    def check_auth_token_format(self, auth_token: str, check_user_existence: bool | None = True) -> None:
        ...
        Returns:
            None.
        """
        if not self.auth_token_manager.check_auth_token_format(
            auth_token=auth_token
        ):
            return ORJSONResponse(
                content={"error": "Bad auth_token format. Please check your auth_token and try again."},
                status_code=400
            )
    ...
    def check_username_existence(self, username: str) -> None:
        ...
        Returns:
            None.
        """
        if self.user_manager.check_username(
            username=username
        ):
            return ORJSONResponse(
                content={"error": "username already exists. Please change it and try again later"},
                status_code=409
            )
    ...
    def check_user_password(self, password: str, user_id: str | None = None, auth_token: str | None = None) -> None:
        """
    ...
    def check_user_suspended(self, user_id: str) -> None:
        ...
        Returns:
            None.
        """
        updated_at = self.database_handler.get_auth_tokens_updated_at(
            user_id=user_id
        )
        ...
        if not is_able_to_update(
            ...
            return ORJSONResponse(
                content={"error": "Cannot reset authentication token. Suspension time has not elapsed."},
                status_code=403
            )
    
    def check_user_privilege(self, user_id: str, check_if_server_owner: bool | None = True, check_if_admin: bool | None = True) -> None:
        """
        ...
        Returns:
            None.
        """
        if (check_if_admin and not self.user_manager.is_admin(
            user_id=user_id
        )) or (check_if_server_owner and not self.user_manager.is_server_owner(
            user_id=user_id
        )):
            return ORJSONResponse(
                status_code=403,
                content={"error": "Access forbidden. Only the server owner and admins can access this route"}
            )
    ...
    def check_channel_id(self, channel_id: str) -> None:
        ...
        Returns:
            None.
        """
        if not self.channels_manager.check_channel(
            ...
            return ORJSONResponse(
                status_code=404,
                content={"error": "The provided channel ID does not exist or could not be found. Please make sure you have entered a valid channel ID and try again."}
            )
    ...
    def check_channel_name(self, channel_name: str) -> None:
        ...
        if channel_name in channels_names:
            return ORJSONResponse(
                status_code=409,
                content={"error": "Channel name already exists, please change it and try again."}
            )
    ...
    def check_message_id(self, message_id: str) -> None:
        ...
        Returns:
            None.
        """
        if not self.messages_manager.check_message(
            message_id=message_id
        ):
            return ORJSONResponse(
                content={"error": "The provided `message_id` is not valid or it doesn't exists, please change it and try again."},
                status_code=404
            )
