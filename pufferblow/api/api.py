...
async def check_if_file_is_protected(file_url: str) -> bool:
    ...
    try:
        ...
        database_uri = (
            str(api_initializer.database.batch_engine.url)
            if hasattr(api_initializer.database, "batch_engine")
            else str(api_initializer.database.database_engine.url)
        )
...
async def create_new_channel_route(request: CreateChannelRequest):
    ...
    is_authorized = api_initializer.user_manager.is_admin(
        user_id=user_id
...
@api.post(
    "/api/v1/channels/{channel_id}/join-audio",
    status_code=200,
...
async def join_voice_channel_route(auth_token: str, channel_id: str):
    ...
    if not api_initializer.channels_manager.check_channel(channel_id):
        logger.warning(
            f"Voice channel not found | User: {user_id} | Channel: {channel_id}"
        )
        raise exceptions.HTTPException(status_code=404, detail="Channel not found")
    ...
    if api_initializer.channels_manager.is_private(channel_id):
        is_authorized = api_initializer.user_manager.is_admin(
            user_id
        ) or api_initializer.user_manager.is_server_owner(user_id)
        if not is_authorized:
            logger.warning(
                f"Access denied to private voice channel | User: {user_id} | Channel: {channel_id}"
            )
            raise exceptions.HTTPException(
                status_code=403, detail="Access denied to private voice channel"
            )
    logger.debug(
        f"Access check passed | User: {user_id} | Channel: {channel_id} | Private: {api_initializer.channels_manager.is_private(channel_id)}"
    )
    ...
    logger.info(
        f"Voice channel WebRTC join attempt | User: {user_id} | Channel: {channel_id} | Checking WebRTC validity"
    )
    ...
    try:
        ...
        logger.info(
            f"Voice channel WebRTC join successful | User: {user_id} | Channel: {channel_id} | Type: {channel_type} | Participants: {participant_count} | ICE servers: {len(result.get('webrtc_config', {}).get('ice_servers', []))}"
        )
        ...
        logger.debug(
            f"Voice channel join response | User: {user_id} | Channel: {channel_id} | Config size: {len(str(response['webrtc_config']))}"
        )
        return response
...
async def leave_voice_channel_route(auth_token: str, channel_id: str):
    ...
    if "error" in result:
        logger.warning(
            f"Voice channel proxy leave failed | User: {user_id} | Channel: {channel_id} | Error: {result['error']}"
        )
        raise exceptions.HTTPException(status_code=400, detail=result["error"])

    logger.info(
        f"Voice channel proxy leave successful | User: {user_id} | Channel: {channel_id}"
    )
...
@api.get(
    "/api/v1/channels/{channel_id}/audio-status",
    status_code=200,
...
async def delete_channel_route(auth_token: str, channel_id: str):
    ...
    # Check if the user is the server admin or server owner
...
@api.get(
    "/api/v1/channels/{channel_id}/load_messages",
    status_code=200,
...
async def channel_load_messages(
    ...
    if api_initializer.channels_manager.is_private(channel_id=channel_id) and (
        not api_initializer.user_manager.is_server_owner(user_id=user_id)
        or not api_initializer.user_manager.is_admin(user_id=user_id)
    ):
    ...
    try:
        ...
        directory_map = {
            "uploads": [
                ...
                "files",
                "uploads",
            ],  # "uploads" should show general uploads + uploads dir directly
            "avatars": ["avatars"],
            "banners": ["banners"],
            "attachments": [
                "images",
                "videos",
                "documents",
            ],  # attachments could be various types
            "stickers": ["stickers"],
            "gifs": ["gifs"],
        ...
        if request.directory == "all":
            ...
            logger.info(
                f"Looking for directory '{request.directory}', mapped to: {subdirectories}"
            )
            ...
            if Path(cdn_base / request.directory).exists():
                ...
                logger.info(
                    f"Direct directory '{request.directory}' exists, added to search list"
                )
                    ...
                    for file_path in files_in_dir:
                        if file_path.is_file():
                            ...
                            try:
                                ...
                                if len(filename_parts) >= 2 and filename_parts[0] in [
                                    "server",
                                    "user",
                                ]:
        ...
        for file_info in all_files_info:
            logger.debug(
                f"File: {file_info['filename']} in {file_info.get('subdirectory', 'unknown')}"
            )
...
async def channels_messages_websocket_legacy(
    ...
    """
    logger.warning(
        f"Legacy WebSocket endpoint used for channel {channel_id} - consider upgrading to /ws"
    )
    ...
    logger.info(
        f"Legacy WebSocket endpoint accessed | User: {user_id} | Channel: {channel_id}"
    )
    ...
    logger.info(
        f"Legacy WebSocket connection established | User: {user_id} | Channel: {channel_id}"
    )
    ...
    try:
        logger.debug(
            f"Starting legacy WebSocket message loop for user {user_id} in channel {channel_id}"
        )
                        ...
                        if message_data.get("type") == "read_confirmation":
                            ...
                            if message_id and message_id in unconfirmed_messages:
                                try:
                                    ...
                                    total_read_confirmations += 1
                                    logger.debug(
                                        f"Legacy: Message marked as read | Channel: {channel_id} | User: {user_id} | Message: {message_id}"
                                    )
                                except Exception as e:
                                    logger.warning(
                                        f"Legacy: Failed to mark message as read | Error: {str(e)}"
                                    )

                    except json.JSONDecodeError as e:
                        logger.warning(
                            f"Legacy: Invalid JSON received | Data: {incoming_data[:100]}... | Error: {str(e)}"
                        )
            ...
            try:
                ...
                for message in latest_messages:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Legacy: Message sent | Channel: {channel_id} | User: {user_id} | Message: {message_id} | Preview: {content_preview}"
                        )

                    except Exception as e:
                        logger.error(
                            f"Legacy: Failed to send message | Error: {str(e)}"
                        )

                if messages_sent_this_cycle > 0:
                    logger.info(
                        f"Legacy: Sent {messages_sent_this_cycle} messages | User: {user_id} | Channel: {channel_id} | Total: {total_messages_sent}"
                    )
        ...
        logger.info(
            f"Legacy WebSocket disconnected | Channel: {channel_id} | User: {user_id} | Code: {e.code} | Reason: {e.reason or 'No reason'} | Stats: sent={total_messages_sent}, confirmed={total_read_confirmations}"
        )
        await api_initializer.websockets_manager.disconnect(websocket)

    except Exception as e:
        logger.error(
            f"Legacy: Unexpected WebSocket error | Error: {str(e)}", exc_info=True
        )
        try:
            await api_initializer.websockets_manager.disconnect(websocket)
        except Exception as disconnect_error:
            logger.error(
                f"Legacy: Error during cleanup | Error: {str(disconnect_error)}"
            )
...
if api_initializer.is_loaded:
    api.mount(
        ...
        StaticFiles(directory=api_initializer.config.CDN_STORAGE_PATH),
        name="cdn",
    )


# Hash-based file serving route
@api.get("/storage/{file_hash}", status_code=200)
async def serve_file_by_hash(file_hash: str):
    """
    Serve a file by its hash. Looks up the file path in the database
    and serves the file directly.
    ...
    try:
        # Look up file in database by hash
...
if api_initializer.is_loaded:
    api.mount(
        ...
        StaticFiles(directory=api_initializer.config.STORAGE_PATH),
        name="storage",
    )
